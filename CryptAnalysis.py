    ...
    def printProb(string, lettersBase, probBase):
        ...
        for letterString in range(len(string)): # loops through each letter in string
            for letterAlph in range(29): # loops through each letter in alphabet
                if lettersBase[letterAlph] == string[letterString]: # checks letter match
                    # increase count for letter
                    letterCount[letterAlph] += 1
                    
                    
        print('Imported Prob:' + '\t' + 'Real Prob:')
        for j in range(29): # loops through alphabet
            letterProb[j] = (letterCount[j] / len(string)) * 100 # calculates probability
            # prints probability of letter in a 'table' comparison to actualy probability
...
def main():
    ...
    printProb(str, asciiLetters, asciiProb)
